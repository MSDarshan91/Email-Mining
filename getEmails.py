...
for person in unique_peeps:
    ...
    for i in range(0, len(unique_subjects.index)):
        ...
        for j in range(0, len(ids.index)):
            ...
            mail_list.append(textUtils.preProcess(emails[ids.ix[j].idd.astype(int)]._body))
...
def getTopWords(level):
	if (level =='peer'):
		...
		for inter in current_user.peer_list:
			print inter.email
			print textUtils.getTopWordsTFIDF(tb(inter.document),doc_list,5)
			print '\n'
